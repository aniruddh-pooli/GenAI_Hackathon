...
<style>
    ...
    /* Custom purple info box for roadmap instructions */
    .purple-info {
    ...
    with st.container():
        ...
        with tab2:
            ...
            if st.session_state.missing_skills_raw and st.session_state.roadmap_steps:
                ...
                st.markdown("""
                <div class="purple-info" style="margin-bottom: 2rem;">
                    <strong>💡 Interactive Roadmap:</strong> Click on any step below to open up the detailed view with projects and resources.
                </div>
                """, unsafe_allow_html=True)
                ...
                for i, step in enumerate(st.session_state.roadmap_steps):
                    col_index = i * 2
                    with cols[col_index]:
                        # Add a container with custom class for targeting
                        ...
                        if st.button(f"**Step {i+1}:** {step_title}", 
                                key=f"step_block_{i}", 
                                use_container_width=True, 
                                help=f"Click for details on Step {i+1}"):
                            st.session_state.selected_step = i
                        st.markdown('</div>', unsafe_allow_html=True)
            
                    if i < len(st.session_state.roadmap_steps) - 1:
                        cols[col_index + 1].markdown(
                            "<div class='step-arrow' style='text-align: center;'>&rarr;</div>", 
                            unsafe_allow_html=True
                        )
